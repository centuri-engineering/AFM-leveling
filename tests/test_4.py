...
from afmprocessing.leveling.leveling import fit_bspline_surface
...
if __name__ == "__main__":
    ...
    file_folder = 'inputs//test_4'
    file_path = os.path.join(file_folder, 'small_patches.txt')
    ...
    tif_file_path = os.path.join(file_folder, 'small_patches.tif')
    ...
    segmented_mask, background_mask = extract_Objects_With_Global_Thresholding(tif_image_data, method='mean', show_figure=True)
    
    
    ##############################
    ### Step 4 - Data leveling ###
    height_data_correction_fitted_surface = fit_bspline_surface(height_data_correction, x_order=3, y_order=3, mask=background_mask) # use background mask to compute the fitted surface
    ...
    height_data_correction_0min = remove_scars(height_data_correction_0min, threshold=0.4, show_figure=True)
