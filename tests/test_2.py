...
from afmprocessing.imagedata.refinement import zero_min_value
from afmprocessing.artefacts.scanline import do_median_of_differences, do_trimmed_mean_of_differences
from afmprocessing.leveling.leveling import fit_polynomial_surface
...
if __name__ == "__main__":
    ...
    file_folder = 'inputs//test_2'
    file_path = os.path.join(file_folder, 'case_3.txt')
    ...
    tif_file_path = os.path.join(file_folder, 'case_3.tif')
    ...
    segmented_mask, background_mask = separate_background_gmm(tif_image_data, n_components=3, background_label=0, show_figure=True)
    
    
    ##############################
    ### Step 4 - Data leveling ###
    height_data_correction_fitted_surface = fit_polynomial_surface(height_data_correction, x_order=1, y_order=1, mask=background_mask) # use background mask to compute the fitted surface
    ...
    height_data_correction_leveled = do_trimmed_mean_of_differences(height_data_correction_leveled, trim_fraction=0.2, method='linear')
